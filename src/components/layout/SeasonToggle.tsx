    ...
    >
      ...
      >
        <AnimatePresence mode="wait" initial={false}>
          ...
          >
            <Snowflake className={`w-3.5 h-3.5 ${isWinter ? 'text-sky-600' : 'text-gray-400'}`} />
          </motion.span>
        </AnimatePresence>
        <span className={`text-xs font-semibold transition-colors duration-200 ${isWinter ? 'text-sky-700' : 'text-gray-400'}`}>
          Winter
        </span>
      </button>
      ...
      >
        <AnimatePresence mode="wait" initial={false}>
          <motion.span
            ...
            className="flex items-center"
          >
            <Sun className={`w-3.5 h-3.5 ${!isWinter ? 'text-amber-500' : 'text-gray-400'}`} />
          </motion.span>
        </AnimatePresence>
        <span className={`text-xs font-semibold transition-colors duration-200 ${!isWinter ? 'text-amber-600' : 'text-gray-400'}`}>
          Summer
        </span>
      </button>
    </div>
